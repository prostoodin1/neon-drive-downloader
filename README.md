...
Beta 7 поставляется универсальным Windows-установщиком, отдельным менеджером
версий и macOS DMG. Официальный Rclone уже включён в приложение и проверяется
по SHA-256 во время сборки.
...
- зелёное, жёлтое и красное свечение карточек шаблонов при наведении;
- отдельное окно монитора Rclone с живым графиком скорости и полным терминалом;
- автоматическая светлая тема днём и тёмная ночью, а также контекстные цвета кнопок;
...
- автозапуск при входе в Windows или macOS и выбор, закрывать ли Neon после фоновой передачи;
...
Для macOS GitHub Actions создаёт отдельные DMG приложения и Installer для
Intel (x64) и Apple Silicon (arm64). Минимальная система — macOS 12 Monterey.
...
4. GitHub Actions загрузит и проверит Rclone, соберёт Windows-установщик,
   отдельный менеджер версий и macOS DMG, затем добавит их в Release.
