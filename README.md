...
- последовательный, ограниченный и полностью параллельный режимы;
...
- **Настройки** — скорость, способ отображения файлов, темы и цвета кнопок,
  фоновая работа, журналы и обновления.

В ограниченном режиме можно выбрать от 2 до 8 одновременных процессов Robocopy.
Режим «все одновременно» запускает отдельный процесс для каждого источника. Это
может увеличить суммарную скорость, но также сильнее нагружает интернет-канал,
диск и клиент Google Drive. Для начала рекомендуется 2–3 одновременные загрузки.
...
3. Создать и отправить тег, например `v5.1.0`.
