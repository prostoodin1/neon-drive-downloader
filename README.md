...
После закрытия приложения отдельный PowerShell-процесс ждёт завершения процесса
PyInstaller, заменяет EXE и запускает его снова.

## Выпуск новой версии

1. Изменить `__version__` в `neon_drive/__init__.py`.
2. Закоммитить и отправить изменения.
3. Создать и отправить тег, например `v5.2.0`.
